...
- A data request that gets an unreadable answer is reported rather than escaping. A proxy or captive portal replying to an API call with HTML, or with JSON that is not an object, raised past the sync loop: no `sync_log` row was written and `doctor` reported a clean log while the cache aged.
- The token file is written atomically. A truncate-then-write left a window in which another host sharing the file could read half of it, which is indistinguishable from a corrupt file and would have been answered by re-authorising.
