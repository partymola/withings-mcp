...
- Refresh failures are now sorted by whether they say anything about the credentials. Only a refusal is an authentication failure: HTTP 400, 401 or 403, or a response-body status that names a credential fault. Everything else is reported as a network error - an unreachable server, a read timeout, a reset connection, a truncated or undecodable response, a 5xx, a rate limit, and any other non-zero body status. The default is deliberate rather than incidental: an authentication failure tells the user to re-authorise, which rewrites a token file other hosts may share, so an unrecognised condition is better under-diagnosed than answered by rotating everyone's token. `withings-mcp auth` classifies its own exchange the same way, instead of letting a failed read escape into the callback handler.
