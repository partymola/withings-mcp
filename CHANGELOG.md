...
- A data request that gets an unreadable answer is reported rather than escaping. A proxy or captive portal replying to an API call with HTML, with a JSON envelope that is not an object, or with an inner body that is present but not an object, raised past the sync loop: no `sync_log` row was written and `doctor` reported a clean log while the cache aged. An inner body that is absent or null is still an empty result, not an error.
- The local page shown during browser sign-in escapes what it displays. One of the messages it shows is built from a parameter in the callback URL, so a crafted value was rendered as markup rather than as text. Reaching it requires the per-run token that the callback also carries, which Withings itself and anything in the redirect chain has.
- No part of a response is repeated back in an error message. A failure carried the status field straight from the response body into the message, which is stored in the sync log and returned to the caller, so anything an intermediary put there was kept and handed on. Only a recognisable status code is quoted now. The interactive sign-in did the same in two places, where the message reaches the terminal and the local browser page, and a transport failure there reported the underlying error's own text - a filesystem path, for a certificate problem.
...
[Unreleased]: https://github.com/partymola/withings-mcp/compare/v0.5.0...HEAD
